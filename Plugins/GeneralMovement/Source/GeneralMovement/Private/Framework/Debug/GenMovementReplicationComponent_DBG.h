...
// These variables are referenced directly by some logging macros in the replication component as well so they must be
// preset whenever logging is enabled (regardless of whether ALLOW_CONSOLE is also defined).
#if !NO_LOGGING

#define DEBUG_NET_CORRECTION_ORIGINAL_CLIENT_LOCATION\
  const FVector Debug_OriginalPawnLocation = PawnOwner->GetActorLocation();
#define DEBUG_NET_CORRECTION_ORIGINAL_CLIENT_ROTATION\
  const FQuat Debug_OriginalPawnRotation = PawnOwner->GetActorRotation().Quaternion();
#define DEBUG_NET_CORRECTION_UPDATED_CLIENT_LOCATION\
  const FVector Debug_UpdatedPawnLocation = PawnOwner->GetActorLocation();
#define DEBUG_NET_CORRECTION_UPDATED_CLIENT_ROTATION\
  const FQuat Debug_UpdatedPawnRotation = PawnOwner->GetActorRotation().Quaternion();
#define DEBUG_NET_CORRECTION_REPLAYED_CLIENT_LOCATION\
  const FVector Debug_ReplayedPawnLocation = PawnOwner->GetActorLocation();
#define DEBUG_NET_CORRECTION_REPLAYED_CLIENT_ROTATION\
  const FQuat Debug_ReplayedPawnRotation = PawnOwner->GetActorRotation().Quaternion();
...
#define DEBUG_SHOW_PAWN_NET_ROLE\
  if (GMCCVars::ShowNetRole != 0 && !IsNetMode(NM_DedicatedServer)) {\
    DrawDebugString(GetWorld(), FVector(0, 0, 150),\
      TEXT("Local Role: ") + DebugGetNetRoleAsString(PawnOwner->GetLocalRole()), PawnOwner, FColor::Red, 0, true);\
    DrawDebugString(GetWorld(), FVector(0, 0, 125),\
      TEXT("Remote Role: ") + DebugGetNetRoleAsString(PawnOwner->GetRemoteRole()), PawnOwner, FColor::Red, 0, true);\
  }

// Display the current simulation delay above the pawn as floating text.
#define DEBUG_SHOW_PAWN_SIMULATION_DELAY\
  if (GMCCVars::ShowSimulationDelay != 0 && !IsNetMode(NM_DedicatedServer)) {\
    if (!IsLocallyControlledServerPawn() && !(IsAutonomousProxy() && bUseClientPrediction)) {\
      DrawDebugString(GetWorld(), FVector(0, 0, 175),\
        TEXT("Sim Delay: ") + FString::SanitizeFloat(SimulationDelay), PawnOwner, FColor::Red, 0, true);\
    }\
  }
...
#define DEBUG_NET_CORRECTION_DRAW_CLIENT_SHAPES\
  if (GMCCVars::ShowClientLocationCorrections != 0) {\
    if (!Debug_OriginalPawnLocation.Equals(Debug_ReplayedPawnLocation, 0.1f)) {\
...
#define DEBUG_LOG_SMOOTHING_INTERPOLATION_DATA\
  if (GMCCVars::LogSmoothing != 0) {\
    ...
    *Method,\
    InterpolationTime)\
    GMC_LOG(Log,\
    TEXT("SmoothingLog: %s --->     InterpolationRatio = %12.3f"),\
    *Method,\
    InterpolationRatio)\
    GMC_LOG(Log,\
    TEXT("SmoothingLog: %s ---> bUsingExtrapolatedData = %s"),\
    *Method,\
    bUsingExtrapolatedData ? TEXT("true") : TEXT("false"))\
    GMC_LOG(Log,\
...
// Log when extrapolated data is being used.
#define DEBUG_LOG_USING_EXTRAPOLATED_DATA\
  if (GMCCVars::LogUsingExtrapolationData != 0 && bUsingExtrapolatedData) {\
    GMC_LOG(Log,\
    TEXT("Using extrapolated state data: start state time = %f | target state time = %f | interpolation ratio = %f | interpolation time = %f | current time = %f"),\
...
#define DEBUG_LOG_SERVER_SAVED_AUTONOMOUS_PROXY_STATE\
  if (GMCCVars::LogClientMoveTrace != 0) {\
    const auto& ServerState = ServerState_AutonomousProxy();\
    GMC_LOG(Log,\
      TEXT("TrackClientMove: Server saved state (quantized)             : Timestamp = %12.6f (ServerWorldTime = %12.6f) |                                  %*s | bFlags = (%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d) |                                                                                                                                                                                                   %*s | Velocity           = %15.6f, %15.6f, %15.6f | Location    = %15.6f, %15.6f, %15.6f | Rotation    = %11.6f, %11.6f, %11.6f | ControlRotation    = %11.6f, %11.6f, %11.6f |"),\
...
#define DEBUG_LOG_NUM_EXECUTED_REMOTE_PACKETS\
  if (GMCCVars::LogNumExecutedRemotePackets != 0) {\
    GMC_LOG(Log,\
      TEXT("LogNumExecutedRemotePackets: Executed %3d packets ---> total packets remaining = %3d (RemoteMoveProcessingChunkSize = %3d) | num exceeding packets this frame = %3d (MaxNumBufferedRemoteMovePackets = %3d)"),\
      NumPacketsExecuted,\
      Server_PendingMovePackets.Num(),\
      RemoteMoveProcessingChunkSize,\
      NumExceedingPackets,\
      MaxNumBufferedRemoteMovePackets)\
  }
#define DEBUG_LOG_NUM_EXECUTED_REMOTE_PACKETS_NONE\
  if (GMCCVars::LogNumExecutedRemotePackets != 0) {\
    GMC_LOG(Log,\
      TEXT("LogNumExecutedRemotePackets: Executed %3d packets ---> total packets remaining = %3d (RemoteMoveProcessingChunkSize = %3d) | num exceeding packets this frame = %3d (MaxNumBufferedRemoteMovePackets = %3d)"),\
      Server_PendingMovePackets.Num(),\
      Server_PendingMovePackets.Num(),\
      RemoteMoveProcessingChunkSize,\
      FMath::Clamp(Server_PendingMovePackets.Num() - MaxNumBufferedRemoteMovePackets, 0, MAX_int32),\
      MaxNumBufferedRemoteMovePackets)\
  }
#define DEBUG_LOG_NUM_EXECUTED_REMOTE_PACKETS_PROXY_MOVE\
  if (GMCCVars::LogNumExecutedRemotePackets != 0) {\
    GMC_LOG(Log,\
      TEXT("LogNumExecutedRemotePackets: Executed  proxy move ---> total packets remaining = %3d (RemoteMoveProcessingChunkSize = %3d) | num exceeding packets this frame = %3d (MaxNumBufferedRemoteMovePackets = %3d)"),\
      Server_PendingMovePackets.Num(),\
      RemoteMoveProcessingChunkSize,\
      FMath::Clamp(Server_PendingMovePackets.Num() - MaxNumBufferedRemoteMovePackets, 0, MAX_int32),\
      MaxNumBufferedRemoteMovePackets)\
