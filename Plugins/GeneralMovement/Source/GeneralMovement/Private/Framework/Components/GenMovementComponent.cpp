...
GENERALMOVEMENT_API DEFINE_LOG_CATEGORY(LogGMCMovement);
...
DECLARE_CYCLE_STAT(TEXT("Auto Resolve Penetration"), STAT_AutoResolvePenetration, STATGROUP_GMCGenMovementComp)
...
void UGenMovementComponent::ReplicatedTick(const FMove& Move, int32 Iteration, bool bIsSubSteppedIteration)
{
  ...
  RotationInput = Move.RotationInput;
  ...
  // Reset the flag for preventing move combination.
  if (Iteration == 1) Client_bDoNotCombineNextMove = false;

  // Reset physics values.
  SetPhysDeltaTime(MoveDeltaTime);

  // Simplified tick function for subclasses.
  FLog(VeryVerbose, "Starting execution of move with timestamp %f (move delta time = %f, sub-stepped iteration = %s).",
    Move.Timestamp, Move.DeltaTime, BOOL2STR(IsSubSteppedIteration()))
  GenReplicatedTick(MoveDeltaTime);
  FLog(VeryVerbose, "Finished execution of move with timestamp %f (move delta time = %f, sub-stepped iteration = %s).",
    Move.Timestamp, Move.DeltaTime, BOOL2STR(IsSubSteppedIteration()))
...
)
{
  ...
  // There's a chance that the state queue is empty and that the smooth states are not yet initialized on the first few frames so we clamp
  // the interpolation time to 0 to avoid negative values.
  InterpolationTime = FMath::Clamp(SmoothState.Timestamp, 0.f, BIG_NUMBER);
...
int32 UGenMovementComponent::Client_ShouldEnqueueMove_Custom_Implementation(const FMove& CurrentMove, const FMove& LastImportantMove) const
{
...
FVector UGenMovementComponent::GetLinearVelocity(UPrimitiveComponent* Component)
{
  ...
  if (IsMovable(Component))
  {
    ...
    // Check if we are based on a rollback actor.
    if (const auto RollbackActor = Cast<AGenRollbackActor>(Owner))
    {
...
FVector UGenMovementComponent::ComputeTangentialVelocity(const FVector& WorldLocation, UPrimitiveComponent* Component)
{
  ...
  {
    // Check if we are based on a rollback actor.
    if (const auto RollbackActor = Cast<AGenRollbackActor>(Component->GetOwner()))
    {
...
bool UGenMovementComponent::UpdateFloor(FFloorParams& Floor, float TraceLength)
{
  ...
  FHitResult ShapeHit = SweepRootCollisionSingleByChannel(
    FVector::DownVector,
    TraceLength,
    FVector::ZeroVector,
    FQuat::Identity,
  ...
  if (ShapeHit.bStartPenetrating)
  {
    ...
    constexpr float MaxSizePenetrationAdjustment = 10.f * UU_CENTIMETER;
    PenetrationAdjustment = PenetrationAdjustment.GetClampedToMaxSize(MaxSizePenetrationAdjustment);
    // The standard implementation usually provides better results in this case.
    bNoAdjustment = !Super::ResolvePenetrationImpl(PenetrationAdjustment, ShapeHit, UpdatedComponent->GetComponentQuat());
    if (!bNoAdjustment)
    {
      FVector ActualAdjustment = UpdatedComponent->GetComponentLocation() - CurrentLocation;
      if (ActualAdjustment.Z > KINDA_SMALL_NUMBER)
      {
        // The pawn's position was adjusted upward. Move back as far as possible so we don't lose contact with the ground.
        MoveUpdatedComponent(
          {0.f, 0.f, -ActualAdjustment.Z}, UpdatedComponent->GetComponentRotation(), true, nullptr, ETeleportType::TeleportPhysics
        );
        ActualAdjustment = UpdatedComponent->GetComponentLocation() - CurrentLocation;
      }
      FLog(
        Verbose,
...
bool UGenMovementComponent::CanMove() const
{
  ...
  return UpdatedComponent->Mobility == EComponentMobility::Movable && Mass >= KINDA_SMALL_NUMBER;
}
...
void UGenMovementComponent::SetRootCollisionLocation(const FVector& NewLocation, ETeleportType Teleport)
{
  if (!HasValidRootCollision()) return;
  UpdatedComponent->SetWorldLocation(NewLocation, false, nullptr, Teleport);
}

FVector UGenMovementComponent::GetRootCollisionLocation() const
{
  if (!HasValidRootCollision()) return FVector{0};
  return UpdatedComponent->GetComponentLocation();
}

void UGenMovementComponent::SetRootCollisionLocationSafe(const FVector& NewLocation, float Tolerance, ETeleportType Teleport)
{
  if (!HasValidRootCollision()) return;

  const FVector InitialLocation = UpdatedComponent->GetComponentLocation();
  if (InitialLocation.Equals(NewLocation, 0.01f))
  {
    return;
  }
...
void UGenMovementComponent::SetRootCollisionRotation(const FQuat& NewRotation, ETeleportType Teleport)
{
  if (!HasValidRootCollision()) return;
  UpdatedComponent->SetWorldRotation(NewRotation, false, nullptr, Teleport);
}

void UGenMovementComponent::SetRootCollisionRotation(const FRotator& NewRotation, ETeleportType Teleport)
{
...
void UGenMovementComponent::SetRootCollisionRotationSafe(const FQuat& NewRotation, float Tolerance, ETeleportType Teleport)
{
  if (!HasValidRootCollision()) return;
...
void UGenMovementComponent::SetRootCollisionRotationSafe(const FRotator& NewRotation, float Tolerance, ETeleportType Teleport)
{
...
FVector UGenMovementComponent::GetValidExtent(EGenCollisionShape CollisionShape, const FVector& Extent) const
{
  ...
  switch (CollisionShape)
  {
    // @UE5 Temporary double casts for interoperability between engine versions 4 and 5.
    case EGenCollisionShape::VerticalCapsule:
    case EGenCollisionShape::HorizontalCapsule:
    {
      ValidExtent.X = ValidExtent.Y = FMath::Max3((double)KINDA_SMALL_NUMBER, (double)Extent.X, (double)Extent.Y);
      ValidExtent.Z = FMath::Max3((double)KINDA_SMALL_NUMBER, (double)Extent.X, (double)Extent.Z);
      break;
    }
    case EGenCollisionShape::Box:
    {
      ValidExtent.X = FMath::Max((double)KINDA_SMALL_NUMBER, (double)Extent.X);
      ValidExtent.Y = FMath::Max((double)KINDA_SMALL_NUMBER, (double)Extent.Y);
      ValidExtent.Z = FMath::Max((double)KINDA_SMALL_NUMBER, (double)Extent.Z);
      break;
    }
    case EGenCollisionShape::Sphere:
    {
      ValidExtent.X = FMath::Max3(Extent.X, Extent.Y, Extent.Z);
      ValidExtent.X = ValidExtent.Y = ValidExtent.Z = FMath::Max((double)KINDA_SMALL_NUMBER, (double)ValidExtent.X);
      break;
    }
    default: checkNoEntryGMC();
  }
...
void UGenMovementComponent::RotateYawTowardsDirection(const FVector& Direction, float Rate, float DeltaTime)
{
...
bool UGenMovementComponent::RotateYawTowardsDirectionSafe(const FVector& Direction, float Rate, float DeltaTime)
{
  ...
  checkGMC(ProposedRotation.IsNormalized())
...
) const
{
  ...
  const FQuat TraceRotation = (Rotation * AddGenCapsuleRotation(UpdatedComponent->GetComponentQuat())).GetNormalized();
  FCollisionQueryParams CollisionQueryParams(FName(__func__), false, GetOwner());
  const auto& CollisionResponseParams = UpdatedComponent->GetCollisionResponseToChannels();
...
FHitResult UGenMovementComponent::AutoResolvePenetration()
{
  ...
  const auto RollbackActor = Cast<AGenRollbackActor>(Hit.GetActor());
  ...
  if (AutoResolvePenetration(RollbackActor, Hit))
  {
  ...
  DEBUG_GMC_LOG_AUTO_RESOLVE_PENETRATION_UP_SWEEP
  SafeMoveUpdatedComponent(-TestDelta, CurrentRotation, true, Hit, ETeleportType::TeleportPhysics);
  DEBUG_GMC_LOG_AUTO_RESOLVE_PENETRATION_DOWN_SWEEP
...
bool UGenMovementComponent::AutoResolvePenetration(AGenRollbackActor* RollbackActor, FHitResult& OutHit)
{
  OutHit = FHitResult{};
  if (!RollbackActor)
  {
  ...
  // @attention Do not call @see SafeMoveUpdatedComponent or anything that ends up calling @see ResolvePenetrationImpl internally in this
  // function (potential infinite recursion).

  // Use the velocity direction of the rollback actor to resolve the penetration.
  constexpr float AdjustmentStepSize = 1.5f;
  const FVector TotalVelocity = RollbackActor->GetVelocity() + RollbackActor->ComputeTangentialVelocity(GetLowerBound());
  const FVector CorrectionDelta = TotalVelocity.GetSafeNormal() * AdjustmentStepSize;
  ...
  constexpr float MaxAdjustDistance = 30.f;

  float TotalAdjustedDistance = 0.f;
  bool bResolvedPenetration = false;
  const FQuat CurrentRotation = UpdatedComponent->GetComponentQuat();
  while (true)
  {
    // Add a small upward correction as well. If we are stuck with the lower part of the collision shape and the actor does not have any
    // upward velocity we can't get out otherwise.
    const float UpCorrection = 0.1f;
    checkGMC(UpCorrection >= 0.f)
    checkGMC(AdjustmentStepSize >= 0.f)
    checkGMC(UpCorrection < AdjustmentStepSize)
    MoveUpdatedComponent(
      CorrectionDelta + FVector{0.f, 0.f, UpCorrection}, CurrentRotation, false, nullptr, ETeleportType::TeleportPhysics
    );
    TotalAdjustedDistance += AdjustmentStepSize + UpCorrection;

    // Check if the penetration was resolved. We move downward by the adjustment step size because otherwise we could lose contact with
    // rollback actors that have upward velocity.
    MoveUpdatedComponent(FVector(0.f, 0.f, -AdjustmentStepSize), CurrentRotation, true, &OutHit, ETeleportType::TeleportPhysics);
...
bool UGenMovementComponent::ResolvePenetrationImpl(const FVector& Adjustment, const FHitResult& Hit, const FQuat& NewRotation)
{
  if (const auto RollbackActor = Cast<AGenRollbackActor>(Hit.GetActor()))
  {
    FHitResult RollbackActorHit;
    if (AutoResolvePenetration(RollbackActor, RollbackActorHit))
    {
      return true;
    }
    ...
    {
      return Super::ResolvePenetrationImpl(GetPenetrationAdjustment(RollbackActorHit), RollbackActorHit, NewRotation);
    }
  }
...
)
{
  ...
  const auto AdjustPosition = [this](float ZAdjustment) {
    FHitResult Hit;
    SafeMoveUpdatedComponent({0.f, 0.f, ZAdjustment}, UpdatedComponent->GetComponentQuat(), true, Hit, ETeleportType::TeleportPhysics);
    return Hit;
  };
...
USceneComponent* UGenMovementComponent::SetRootCollisionShape(EGenCollisionShape NewCollisionShape, const FVector& Extent, FName Name)
{
  ...
  {
    // The root component already has the requested collision shape, only update the extent.
...
) const
{
  ...
  FQuat TraceRotation = CollisionShape == EGenCollisionShape::HorizontalCapsule ?
    Rotation * UFlatCapsuleComponent::FLAT_ROTATION_DEFAULT.Quaternion() : Rotation;
  FCollisionQueryParams CollisionQueryParams(FName(__func__), false, GetOwner());
  const auto& CollisionResponseParams = UpdatedComponent->GetCollisionResponseToChannels();
...
bool UGenMovementComponent::StepMontage(USkeletalMeshComponent* Mesh, UAnimMontage* Montage, float Position, float Weight, float PlayRate)
{
  ...
  // @attention Auto blend out can cause erroneous behaviour with manual montage stepping so it must be disabled. However, since stepped
  // montages should be cleared after each tick anyway this won't cause any unexpected behaviour.
...
float UGenMovementComponent::GetMontagePosition(USkeletalMeshComponent* Mesh, UAnimMontage* Montage) const
{
  ...
  // Sometimes there is a slight deviation between server and client after a montage was updated in the pose tick, e.g. on the server the
  // montage could still be in the instance array even when it has already ended (the position being equal to the montage length) while on
  // the client the montage could already be removed from the array. Therefore we also want to return -1 when the position is at the end of
  // the montage to remain consistent across server and client.
  const auto MakeNetSafe = [](float MontagePosition, UAnimMontage* MontageAsset) {
    if (MontageAsset)
    {
      return FMath::IsNearlyEqual(MontagePosition, MontageAsset->GetPlayLength(), 0.000001f) ? -1.f : MontagePosition;
    }
    return -1.f;
  };
  ...
  if (Montage)
  {
    ...
    for (int32 Index = MontageInstances.Num() - 1; Index >= 0; --Index)
    {
      ...
      if (MontageInstance && Montage == MontageAsset)
      {
        return MakeNetSafe(MontageInstance->GetPosition(), MontageAsset);
      }
    }
    return -1.f;
  }
  ...
  for (int32 Index = MontageInstances.Num() - 1; Index >= 0; --Index)
  {
    ...
    if (MontageInstance)
    {
      return MakeNetSafe(MontageInstance->GetPosition(), MontageInstance->Montage/*may be nullptr*/);
    }
  }
...
float UGenMovementComponent::GetMontageWeight(USkeletalMeshComponent* Mesh, UAnimMontage* Montage) const
{
  ...
  // montage could still be in the instance array even when it has already ended (the position being equal to the montage length) while on
  // the client the montage could already be removed from the array. Therefore we also want to return -1 when the position is at the end of
  // the montage (the weight would most likely be 0 in that case) to remain consistent across server and client.
  const auto MakeNetSafe = [](float MontageWeight, float MontagePosition, UAnimMontage* MontageAsset) {
    if (MontageAsset)
    {
      return FMath::IsNearlyEqual(MontagePosition, MontageAsset->GetPlayLength(), 0.000001f) ? -1.f : MontageWeight;
    }
    return -1.f;
  };
  ...
  if (Montage)
  {
    ...
    for (int32 Index = MontageInstances.Num() - 1; Index >= 0; --Index)
    {
      ...
      if (MontageInstance && Montage == MontageAsset)
      {
        return MakeNetSafe(MontageInstance->GetWeight(), MontageInstance->GetPosition(), MontageAsset);
      }
    }
    return -1.f;
  }
  ...
  for (int32 Index = MontageInstances.Num() - 1; Index >= 0; --Index)
  {
    ...
    if (MontageInstance)
    {
      return MakeNetSafe(MontageInstance->GetWeight(), MontageInstance->GetPosition(), MontageInstance->Montage/*may be nullptr*/);
    }
  }
...
void UGenMovementComponent::UpdateMontageParameters(
  float& OutPosition,
  float& OutWeight,
...
{
  OutPosition = GetMontagePosition(Mesh, Montage);
  OutWeight = GetMontageWeight(Mesh, Montage);
}

void UGenMovementComponent::ResetAllMontages(USkeletalMeshComponent* Mesh) const
{
  if (!Mesh) return;
...
void UGenMovementComponent::ResetRootMotionMontage(USkeletalMeshComponent* Mesh) const
{
  if (!Mesh) return;
  ...
  const auto RootMotionMontageInstance = AnimInstance->GetRootMotionMontageInstance();
  if (!RootMotionMontageInstance) return;

  // Sets @see UAnimInstance::RootMotionMontageInstance to null as well.
  AnimInstance->ClearMontageInstanceReferences(*RootMotionMontageInstance);
  AnimInstance->MontageInstances.Remove(RootMotionMontageInstance);
  delete RootMotionMontageInstance;
}
...
void UGenMovementComponent::PausePathFollowing(FAIRequestID RequestID, bool bKeepVelocity)
{
  if (!PawnOwner)
  {
    return;
  ...
  // If "bKeepVelocity" is false this will call @see StopActiveMovement internally which will then zero all velocity related members.
...
void UGenMovementComponent::ResumePathFollowing(FAIRequestID RequestID)
{
  if (!PawnOwner)
  {
    return;
...
void UGenMovementComponent::StopActiveMovement()
{
  Super::StopActiveMovement();
  HaltMovement();
}

FAIRequestID UGenMovementComponent::GetCurrentPathFollowingRequest() const
{
...
void UGenMovementComponent::DoNotCombineNextMove()
{
