...
#define DEFINE_PREREPLICATED_DATA_FMOVE()\
  ...
  ADD_PREREPLICATED_TYPE_TO_MOVE(double,           Double)\
...
#define DEFINE_PREREPLICATED_DATA_FSTATE()\
  ...
  ADD_PREREPLICATED_TYPE_TO_STATE(double,           Double)\
...
#define DEFINE_PREREPLICATED_DATA_FSTATEREDUCED()\
  ...
  ADD_PREREPLICATED_TYPE_TO_REDUCED_STATE(double,           Double)\
...
#define IMPLEMENT_REPLICATION_SYSTEM()\
  ...
  ADD_REPLICATION_LOGIC(double,           Double)\
...
#define SwapStateBufferBoundData_IMPLEMENTATION()\
  ...
  CALL_SwapStateBufferBoundData(Double)\
...
#define ForceNetUpdateCheckBoundData_IMPLEMENTATION()\
  ...
  CALL_ForceNetUpdateCheckBoundData(Double)\
...
#define SaveBoundDataToServerState_IMPLEMENTATION()\
  ...
  CALL_SaveBoundDataToServerState(Double)\
...
#define ResetLockedBoundData_IMPLEMENTATION()\
  ...
  CALL_ResetLockedBoundData(Double)\
...
#define LoadBoundDataForReplay_IMPLEMENTATION()\
  ...
  CALL_LoadBoundDataForReplay(Double)\
...
#define SaveInBoundDataToMove_IMPLEMENTATION()\
  ...
  CALL_SaveInBoundDataToMove(Double)\
...
#define SaveOutBoundDataToMove_IMPLEMENTATION()\
  ...
  CALL_SaveOutBoundDataToMove(Double)\
...
#define SaveBoundDataToState_IMPLEMENTATION()\
  ...
  CALL_SaveBoundDataToState(Double)\
...
#define LoadInBoundDataFromMove_IMPLEMENTATION()\
  ...
  CALL_LoadInBoundDataFromMove(Double)\
...
#define LoadBoundDataFromState_IMPLEMENTATION()\
  ...
  CALL_LoadBoundDataFromState(Double)\
...
#define LoadReplicatedBoundDataFromState_IMPLEMENTATION()\
  ...
  CALL_LoadReplicatedBoundDataFromState(Double)\
...
#define AddTargetStateBoundDataToInitState_IMPLEMENTATION()\
  ...
  CALL_AddTargetStateBoundDataToInitState(Double)\
...
#define AddStartStateBoundDataToInitState_IMPLEMENTATION()\
  ...
  CALL_AddStartStateBoundDataToInitState(Double)\
...
#define IsBoundDataValid_IMPLEMENTATION()\
  ...
  CALL_IsBoundDataValidSpecific(Double)\
...
#define SerializeBoundData_IMPLEMENTATION()\
  ...
  CALL_SerializeBoundDataGeneric(Double)\
...
UENUM(BlueprintType)
enum class EPreReplicatedDouble : uint8
{ Double1, Double2, Double3, Double4, Double5, Double6, Double7, Double8, Double9, Double10, Double11, Double12, Double13, Double14, Double15, Double16 };
UENUM(BlueprintType)
...
#define CALL_ForceNetUpdateCheckBoundData(Name)       if (Server_ForceNetUpdateCheck##Name(ServerState, RecipientRole)) bShouldForceNetUpdate = true;
#define CALL_SaveBoundDataToServerState(Name)         Server_Save##Name##ToServerState(ServerState, RecipientRole, bUsingClientPrediction);
...
private:\
  ...
  bool Server_ForceNetUpdateCheck##Name(FState& ServerState, ENetRole RecipientRole)\
  {\
    const auto& LastServerState = RecipientRole == ROLE_SimulatedProxy ?\
      ServerState_SimulatedProxy_Buffered() : ServerState_AutonomousProxy_Buffered();\
    bool bShouldForceNetUpdate = false;\
    const float CurrentTime = GetTime();\
    if (Name##1)  { if (ServerState.bReplicate##Name##1  && ServerState.bForceNetUpdate##Name##1  && !bLocked##Name##1)  { if (ServerState.Name##1  != LastServerState.Name##1)  { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##1  = true; LockSetTime##Name##1  = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##2)  { if (ServerState.bReplicate##Name##2  && ServerState.bForceNetUpdate##Name##2  && !bLocked##Name##2)  { if (ServerState.Name##2  != LastServerState.Name##2)  { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##2  = true; LockSetTime##Name##2  = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##3)  { if (ServerState.bReplicate##Name##3  && ServerState.bForceNetUpdate##Name##3  && !bLocked##Name##3)  { if (ServerState.Name##3  != LastServerState.Name##3)  { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##3  = true; LockSetTime##Name##3  = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##4)  { if (ServerState.bReplicate##Name##4  && ServerState.bForceNetUpdate##Name##4  && !bLocked##Name##4)  { if (ServerState.Name##4  != LastServerState.Name##4)  { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##4  = true; LockSetTime##Name##4  = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##5)  { if (ServerState.bReplicate##Name##5  && ServerState.bForceNetUpdate##Name##5  && !bLocked##Name##5)  { if (ServerState.Name##5  != LastServerState.Name##5)  { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##5  = true; LockSetTime##Name##5  = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##6)  { if (ServerState.bReplicate##Name##6  && ServerState.bForceNetUpdate##Name##6  && !bLocked##Name##6)  { if (ServerState.Name##6  != LastServerState.Name##6)  { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##6  = true; LockSetTime##Name##6  = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##7)  { if (ServerState.bReplicate##Name##7  && ServerState.bForceNetUpdate##Name##7  && !bLocked##Name##7)  { if (ServerState.Name##7  != LastServerState.Name##7)  { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##7  = true; LockSetTime##Name##7  = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##8)  { if (ServerState.bReplicate##Name##8  && ServerState.bForceNetUpdate##Name##8  && !bLocked##Name##8)  { if (ServerState.Name##8  != LastServerState.Name##8)  { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##8  = true; LockSetTime##Name##8  = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##9)  { if (ServerState.bReplicate##Name##9  && ServerState.bForceNetUpdate##Name##9  && !bLocked##Name##9)  { if (ServerState.Name##9  != LastServerState.Name##9)  { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##9  = true; LockSetTime##Name##9  = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##10) { if (ServerState.bReplicate##Name##10 && ServerState.bForceNetUpdate##Name##10 && !bLocked##Name##10) { if (ServerState.Name##10 != LastServerState.Name##10) { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##10 = true; LockSetTime##Name##10 = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##11) { if (ServerState.bReplicate##Name##11 && ServerState.bForceNetUpdate##Name##11 && !bLocked##Name##11) { if (ServerState.Name##11 != LastServerState.Name##11) { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##11 = true; LockSetTime##Name##11 = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##12) { if (ServerState.bReplicate##Name##12 && ServerState.bForceNetUpdate##Name##12 && !bLocked##Name##12) { if (ServerState.Name##12 != LastServerState.Name##12) { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##12 = true; LockSetTime##Name##12 = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##13) { if (ServerState.bReplicate##Name##13 && ServerState.bForceNetUpdate##Name##13 && !bLocked##Name##13) { if (ServerState.Name##13 != LastServerState.Name##13) { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##13 = true; LockSetTime##Name##13 = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##14) { if (ServerState.bReplicate##Name##14 && ServerState.bForceNetUpdate##Name##14 && !bLocked##Name##14) { if (ServerState.Name##14 != LastServerState.Name##14) { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##14 = true; LockSetTime##Name##14 = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##15) { if (ServerState.bReplicate##Name##15 && ServerState.bForceNetUpdate##Name##15 && !bLocked##Name##15) { if (ServerState.Name##15 != LastServerState.Name##15) { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##15 = true; LockSetTime##Name##15 = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    if (Name##16) { if (ServerState.bReplicate##Name##16 && ServerState.bForceNetUpdate##Name##16 && !bLocked##Name##16) { if (ServerState.Name##16 != LastServerState.Name##16) { bShouldForceNetUpdate = true; if (MinRepHoldTime > 0.f) { bLocked##Name##16 = true; LockSetTime##Name##16 = CurrentTime; } } } } else return bShouldForceNetUpdate;\
    return bShouldForceNetUpdate;\
  }\
  void Server_Save##Name##ToServerState(FState& ServerState, ENetRole RecipientRole, bool bUsingClientPrediction) const\
  {\
    const bool bIsAutonomousProxyRecipient = RecipientRole == ROLE_AutonomousProxy;\
    if (bIsAutonomousProxyRecipient && bUsingClientPrediction)\
    {\
...
// Generic bound data validation. For floating point numbers a specific validation function should be implemented.
...
#define IMPLEMENT_CUSTOM_SERIALIZATION_SETTINGS(TypeName, MemberName, RPCName)\
  ...
  void RPCName##_Implementation2(const TArray<TypeName>& RemoteMoves)\
  {\
    check(sizeof(TypeName) == sizeof(FMove) && std::is_standard_layout<TypeName>::value && std::is_standard_layout<FMove>::value)\
    const TArray<FMove>& ReceivedMoves = reinterpret_cast<const TArray<FMove>&>(RemoteMoves);\
    Server_ProcessClientMoves(ReceivedMoves);\
  }\
  bool RPCName##_Validate2(const TArray<TypeName>& RemoteMoves)\
  {\
    return true;\
  }
