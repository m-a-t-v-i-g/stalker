...
GENERALMOVEMENT_API DECLARE_LOG_CATEGORY_EXTERN(LogGMCPawn, Log, All);
...
class GENERALMOVEMENT_API AGenPawn : public APawn
{
  GENERATED_BODY()
...
public:
  ...
  /// Flag to signal to the replication component that the pawn requires a full net serialization update (e.g. after it was possessed by a
  /// a new controller). The flag should be reset by the replication component itself after it was processed.
  bool Server_bNeedsFullNetSerializationOnNextUpdate{false};
