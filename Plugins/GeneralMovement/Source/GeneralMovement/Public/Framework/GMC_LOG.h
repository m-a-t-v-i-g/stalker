...
// Returns the net mode enum value as FString.
FORCEINLINE FString DebugGetNetModeAsString(ENetMode Mode)
{
  switch (Mode)
  {
...
#ifdef GMC_ROLLBACK_ACTOR_LOG
#define ACTOR_REF this
#define LOG_CATEGORY LogGMCRollbackActor
#endif

#if defined PAWN_REF && defined LOG_CATEGORY
// Logging macro wrapper, prepends additional debug info about pawns.
#define GMC_LOG(Verbosity, Format, ...)\
  {\
    if (PAWN_REF) {\
      ENetMode gmc_Mode = GetNetMode();\
      FString\
      gmc_Time = FDateTime::Now().GetTimeOfDay().ToString(),\
      gmc_PlayerName = PAWN_REF->GetHumanReadableName(),\
      gmc_ActorName = PAWN_REF->GetName(),\
      gmc_NetMode = DebugGetNetModeAsString(gmc_Mode),\
      ...
      LOG(LOG_CATEGORY, Verbosity, OBJECT_INFO_TEXT(Format),\
        OBJECT_INFO(gmc_Time, gmc_PlayerName, gmc_ActorName, gmc_NetMode, gmc_NetworkRole), ##__VA_ARGS__)\
    }\
    else {\
...
#elif defined ACTOR_REF && defined LOG_CATEGORY
// Logging macro wrapper, prepends additional debug info about rollback actors.
#define GMC_LOG(Verbosity, Format, ...)\
  {\
    if (ACTOR_REF) {\
      ENetRole gmc_Role = ACTOR_REF->GetLocalRole();\
      ENetMode gmc_Mode = GetNetMode();\
      APlayerController* gmc_LocalPC = GEngine ? GEngine->GetFirstLocalPlayerController(GetWorld()) : nullptr;\
      FString\
      gmc_Time = FDateTime::Now().GetTimeOfDay().ToString(),\
      gmc_PlayerName = gmc_LocalPC ? gmc_LocalPC->GetHumanReadableName() : TEXT(""),\
      gmc_ActorName = ACTOR_REF->GetHumanReadableName(),\
      gmc_NetMode = DebugGetNetModeAsString(gmc_Mode),\
      gmc_NetworkRole = DebugGetNetRoleAsString(gmc_Role);\
      LOG(LOG_CATEGORY, Verbosity, OBJECT_INFO_TEXT(Format),\
        OBJECT_INFO(gmc_Time, gmc_PlayerName, gmc_ActorName, gmc_NetMode, gmc_NetworkRole), ##__VA_ARGS__)\
      ...
      LOG(LOG_CATEGORY, Error, TEXT("| GMC_LOG failed, ACTOR_REF was nullptr | FORMAT-MSG: ") Format, ##__VA_ARGS__)\
    }\
  }
#define GMC_CLOG(Condition, Verbosity, Format, ...)\
  if (Condition) {\
...
#define LOG(Category, Verbosity, Format, ...) UE_LOG(Category, Verbosity, Format, ##__VA_ARGS__)
#else
// Use the standard log if no actor reference or logging category was set (using "LogTemp" as logging category instead).
...
#define OBJECT_INFO(Time, FileName, PlayerName, ActorName, NetMode, NetworkRole)
...
// to GMC.Build.cs to enable. Only works in build configurations that have "DO_CHECK" enabled as well.
#if DO_CHECK && defined GMC_DO_CHECK_EXTENSIVE
