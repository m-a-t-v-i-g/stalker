...
GENERALMOVEMENT_API DECLARE_LOG_CATEGORY_EXTERN(LogGMCMovement, Log, All);
...
struct GENERALMOVEMENT_API FFloorParams
{
  GENERATED_BODY()
...
class GENERALMOVEMENT_API UGenMovementComponent : public UGenMovementReplicationComponent
{
  GENERATED_BODY()
...
public:
  ...
  /// Returns the rotation input of the move currently being executed. This is the raw rotation input reflecting the physical user input.
  ///
  /// @returns      FRotator    The view rotation input of the current move.
  UFUNCTION(BlueprintCallable, Category = "General Movement Component")
  FRotator GetMoveRotationInput() const;
  ...
  /// The timestamp of the smoothed pawn state i.e. the time at which the simulated pawn is currently being displayed.
  ///
  /// @returns      float    The current interpolation time.
  UFUNCTION(BlueprintCallable, Category = "General Movement Component")
  float GetCurrentInterpolationTime() const;
...
#pragma region Physics
  ...
  /// Adds an impulse to the pawn. An impulse should only be applied for one single frame. If you want to add a continous force to the pawn
  /// over several frames use @see AddForce.
  ///
  ...
  /// Adds a radial impulse to the pawn. An impulse should only be applied for one single frame. If you want to add a continous radial force
  /// to the pawn over several frames use @see AddRadialForce.
  ///
...
#pragma endregion
  ...
  /// Returns the width of the root component. This is the extent in the XY plane from the center in the given direction. For uniform shapes
  /// the return value is always the radius (meaning the passed direction does not matter) but for horizontal capsules and box shapes this
  /// is the distance to the boundary of the collision in the passed direction.
  ///
  ...
  /// Version of @see SetRootCollisionLocation that only sets the new location if it will not cause any blocking collision.
  ///
  /// @param        NewLocation    The new location of the root collision in world space.
  /// @param        Tolerance      When testing whether the new location would cause a blocking collision, this argument is applied as a
  ...
  /// @param        Hit              The most recent hit that resulted from checking if the penetration was resolved.
  /// @returns      bool             True if the penetration could be resolved, false otherwise.
  virtual bool AutoResolvePenetration(class AGenRollbackActor* RollbackActor, FHitResult& OutHit);

protected:

  /// For some reason shape traces and sweeps involving movable actors (such as the rollback actors) can produce inconsistent results
  /// between server and client even when the actor positions and the trace start and end points are exactly the same. To work around the
  /// issue we override this function to not resolve penetrations based on the passed hit result when a rollback actor was hit. Although
  /// this doesn't guarantee perfect client-server synchronisation as the hit actor itself can differ as well, it does work reasonably well
  /// in most situations. From empirical tests the hit normals provide the least consistent values.
  ///
  /// @param        Adjustment    The requested adjustment, usually obtained from @see UMovementComponent::GetPenetrationAdjustment.
...
public:
  ...
  /// @attention This adds the velocity via @see AddImpulse.
  ///
  ...
  /// @returns      void
  UFUNCTION(BlueprintCallable, Category = "General Movement Component")
  virtual void ImpartVelocityFrom(UPrimitiveComponent* Source, bool bAddLinearVelocity, bool bAddAngularVelocity, bool bVelChange);
  ...
  /// collision, physics, rendering etc.) will be adopted and the original root component will be destroyed. Only updates the extent if the
  /// root component already has the passed collision shape.
  ...
  /// @param        Name                 The name of the new root component (must be unique). The name will not be updated if the root
  ///                                    component already has the passed collision shape.
  ...
  /// in the next tick to drive the montage further. The function allocates a new montage with each call so all replicated montage data
  /// should be reset before the montage is stepped again in the next tick (@see ResetAllMontages, @see ResetRootMotionMontage). This will
  /// also enforce frame independence for any other montage related data in the anim instance (important for client replay).
  ...
  /// Saves the current position and weight of the passed montage (if playing) to the out variables. If "Montage" is nullptr the parameters
  /// of the first found currently playing montage will be queried.
  ///
  /// @param        OutPosition    The variable to save the montage position to.
  /// @param        OutWeight      The variable to save the montage weight to.
  /// @param        Mesh           The skeletal mesh of the owning pawn.
  /// @param        Montage        The montage to query the position and weight for. If nullptr the first found currently playing montage
  ///                              will be queried.
  /// @returns      void
  UFUNCTION(BlueprintCallable, BlueprintPure = false, Category = "General Movement Component")
  virtual void UpdateMontageParameters(
    UPARAM(ref, DisplayName = "Position") float& OutPosition,
    UPARAM(ref, DisplayName = "Weight") float& OutWeight,
  ...
  /// @returns      void
  UFUNCTION(BlueprintCallable, Category = "General Movement Component")
  virtual void ResetAllMontages(USkeletalMeshComponent* Mesh) const;

  /// Clears the currently active root motion montage (if any) from @see UAnimInstance::MontageInstances and
  /// @see UAnimInstance::ActiveMontagesMap.
  ///
  /// @param        Mesh    The skeletal mesh of the owning pawn.
  /// @returns      void
  UFUNCTION(BlueprintCallable, Category = "General Movement Component")
  virtual void ResetRootMotionMontage(USkeletalMeshComponent* Mesh) const;
  ...
  UFUNCTION(BlueprintCallable, Category = "General Movement Component")
  /// Calling this function will force the next move to be enqueued as a new entry in the move queue. Only relevant for autonomous proxies,
  /// this function does nothing for any other kind of pawn.
  void DoNotCombineNextMove();
...
private:
  ...
  /// The timestamp of the smoothed pawn state.
...
FORCEINLINE FRotator UGenMovementComponent::GetMoveRotationInput() const
{
...
FORCEINLINE float UGenMovementComponent::GetCurrentInterpolationTime() const
{
