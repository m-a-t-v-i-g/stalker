...
void UOrganicAnimInstance::NativeInitializeAnimation()
{
	...
	Character = Cast<ABaseOrganic>(TryGetPawnOwner());
	if (Character)
	{
...
void UOrganicAnimInstance::NativeBeginPlay()
{
...
void UOrganicAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	CharacterInformation.Rotation = Character->GetOrganicMovement()->GetRootCollisionRotation();
	CharacterInformation.ViewRotation = Character->GetOrganicViewRotation();
	CharacterInformation.AimYawRate = Character->GetViewYawRate();
	
	CharacterInformation.Velocity = Character->GetOrganicMovement()->GetVelocity();
	CharacterInformation.Acceleration = Character->GetAcceleration();
	CharacterInformation.Speed = Character->GetSpeed();
	
	//CharacterInformation.TransientAcceleration = Character->GetTransientAcceleration();
	...
	if (MovementState.Grounded())
	{
		...
		if (bPreviousShouldMove == false && Grounded.bShouldMove)
		{
...
FVector UOrganicAnimInstance::CalculateRelativeAccelerationAmount() const
{
	...
	{
		const float MaxAcc = Character->GetOrganicMovement()->GetInputAcceleration();
		return CharacterInformation.Rotation.UnrotateVector(
			CharacterInformation.Acceleration.GetClampedToMaxSize(MaxAcc) / MaxAcc);
	}

	const float MaxBrakingDec = Character->GetOrganicMovement()->GetBrakingDeceleration();
	return
